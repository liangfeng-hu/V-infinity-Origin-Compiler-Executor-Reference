# VC 10页简版 Deck｜YFCore PoAIW（Gate 91 + LSE）
**版本/日期：** DistillGuard-Core v1.0｜2026-02-24  
**公司/团队：** YFCore Technology Limited  
**一句话定位：** 把“模型蒸馏/模型提取”从软件猫鼠游戏，抬升为**物理不可伪造**的运行态输出主权体系（Fail-Closed）。

---

## 01｜封面
- **标题：** PoAIW：Physical Proof of AI Work（AI 物理做功证明）
- **副标题：** Gate 91（熵不可克隆）+ LSE（叠加涌现支持定律）  
  ——把“模型蒸馏”从软件对抗抬升为物理不可伪造
- **公司/团队：** YFCore Technology Limited
- **版本/日期：** DistillGuard-Core v1.0｜2026-02-24

---

## 02｜行业痛点：蒸馏/模型提取已进入工业化
...
- **公开证据锚点（页脚）：**
  ...
  - Google/GTIG 指出模型提取（distillation/model extraction）在上升，并披露“>10 万提示”级别提取活动。[Google Cloud][google-gtig]

---

## 03｜根因：输出没有“物理归属”，所以软件防御必然漏风
...
### 对标框（放在本页底部，帮助高管10秒定位）
| 维度 | Claude Code Security（Anthropic） | PoAIW / DistillGuard（YFCore） |
|---|---|---|
| 防御维度 | 软件/语义/逻辑层（DevSecOps） | 软件 + 时间 + 物理层（Runtime） |
| 核心目标 | **防漏洞**：扫描代码漏洞并建议补丁（部署前） | **防窃取**：阻断高保真输出数据集采集（推理时） |
| 发生阶段 | Pre-Deployment（合并/部署前） | Pre-Commit 网关（输出端） |
| 对工业蒸馏 | 基本无直接防护 | 通过物理做功证明 + Fail-Closed 改写可行性 |
> Claude Code Security 的公开定位：在 Claude Code 中扫描代码库漏洞并给补丁建议（研究预览）。[Anthropic][anthropic-claude-code-security]

---

## 04｜范式跃迁：PoAIW（AI 物理做功证明）
- **核心思想：** 不再主要审“语义”，而是审“做功证明（Proof of Work/Entropy）”。
- **定义：** 每一次“高保真/高价值输出”必须携带可验证的硬件根信任证明（TEE/机密计算/签名证明），并与能耗/熵增相关证据闭包绑定。
- **类比一句话（给高管听）：** 从“防洗稿”升级为“印钞机级防伪”——不是识别坏人，而是要求**不可伪造的物理收据**。

---

## 05｜最小可交付产品：Gate 91 + LSE（最锋利刀尖）
### Gate 91：I_ENTROPY_CLONE（物理熵不可克隆硬门）
- **要点：** 高等级输出必须满足：
  - ZK 验证通过
  - 硬件证明通过（TEE/Attestation）
  - 热力学/能耗相关门为 0  
  否则直接 Fail-Closed（ShadowOnly/归零）。

### LSE：叠加涌现支持定律（第0元门）
- **要点：** 意图层 × 时间连续 × 物理层必须**同时成立**才允许“涌现/高保真输出”。
- **三条支撑语义（元自护）：**
  - 验证器可信（Verifier trust）
  - 预算自护（Energy budget self-protect）
  - 影子隔离（Shadow isolation）
- **价值：** 形成闭环故事线：不仅能拦截攻击，还能证明防线未被绕过（自洽性可审计）。

---

## 06｜集成方式：不重训模型，做成可插拔“高安全执行内核”
- **插入点（工程语言）：** 在 API 输出端/网关 **Pre-Commit** 阶段加一层“门控与收据闭包”。
- **依赖：** 复用既有 TEE/机密计算/签名验证/能耗计量能力；对现有推理逻辑低侵入。
- **交付形态：**
  1) 参考实现（开源）：Gate 91 + LSE 的形式化规范 + 验证器目录 + 演示  
  2) 生产实现（合作/NDA）：硬件绑定、证据链、审计与性能工程化

---

## 07｜客户为什么会买：把“资产流失”变成“物理不可能”
- **对模型厂：** 显著抬高蒸馏的边际成本（拿不到硬件做功证明就拿不到高保真语料）。
...
- **宏观背书：** 国际 AI 安全报告（2026）系统性讨论能力、风险与治理框架需求。[International AI Safety Report][iasr-2026]
...
- **升级路径（Upsell）：**  
  Gate 91 + LSE 先落地；之后再引入 Gate 90（意图/提取风险/种子连续性）等更完整门阵列，形成 Trinity（90+91+LSE）成本不对称防御。
...
[reuters-openai]: https://www.reuters.com/world/china/openai-accuses-deepseek-distilling-us-models-gain-advantage-bloomberg-news-2026-02-12/ "Reuters: OpenAI says DeepSeek distilled US models (Feb 12, 2026)"
...
[reuters-anthropic-distill]: https://www.reuters.com/world/china/chinese-companies-used-claude-improve-own-models-anthropic-says-2026-02-23/ "Reuters: Chinese firms used Claude via distillation (Feb 23, 2026)"
...
[iasr-2026]: https://internationalaisafetyreport.org/publication/international-ai-safety-report-2026 "International AI Safety Report 2026 (Feb 3, 2026)"
